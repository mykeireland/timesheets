...
namespace Timesheets.Functions
{
    public static class ApproveTimesheet
    {
        ...
        public static async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Function, "post", Route = "timesheets/approve")] HttpRequest req,
            ILogger log)
        {
            log.LogInformation("Processing timesheet approval...");
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    return new OkObjectResult(new { Updated = rows });
                }
            }
        }
    }
